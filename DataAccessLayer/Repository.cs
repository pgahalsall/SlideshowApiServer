...
namespace SlideshowDataAccess
{
        public class Repository : IRepository, IDisposable
        {
            ...
            private SlideServerDbEntities _dbContext { get; set; }

            public Repository(SlideServerDbEntities dbContext)
            {
        ...
        {
            using (SlideServerDbEntities entities = new SlideServerDbEntities())
            {
                IEnumerable<Slide> slides = entities.Slides.ToList();
                return slides;
            }
        ...
        {
            return _dbContext.Slides.Where(s => s.slide_id == slideId).FirstOrDefault();
        }

        public Slideshow GetSlideshow(int slideshowid)
        {
            Slideshow slideshow = _dbContext.Slideshows
                             .Where(sh => sh.slideshow_id == slideshowid).FirstOrDefault();

            return slideshow;
        }

        public IEnumerable<Slideshow> GetAllSlideshows()
        {
            IEnumerable<Slideshow> slideshows = _dbContext.Slideshows;

            return slideshows;
        }

        public IEnumerable<Slide> GetSlideshowSlides(int slideshowId)
        {
            List<Slide> slideList = new List<Slide>();

            List<SlideshowSlide> slides = _dbContext.SlideshowSlides
                               .Where(sss => sss.slideshow_id == slideshowId)
                               .ToList();

            foreach(SlideshowSlide sss in slides)
            {
                List<Slide> aList = _dbContext.Slides.Where(s => s.slide_id == sss.slide_id).ToList();
                slideList.AddRange(aList);
            }
        ...
        // Soundtracks
        public IEnumerable<Soundtrack> GetAllSoundtracks()
        {
            IEnumerable<Soundtrack> sounds = _dbContext.Soundtracks;
        ...
        {
            Soundtrack sound = _dbContext.Soundtracks
                             .Where(st => st.soundtrack_id == id).FirstOrDefault();
        ...
        public User GetUser(int id)
        {
            User user= _dbContext.Users
                             .Where(u => u.user_id == id).FirstOrDefault();
        ...
        //        return null;
        //    }
        //    //using (SlideshowContext entities = new SlideshowContext())
        //    //{
