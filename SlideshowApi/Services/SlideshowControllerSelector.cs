...
{
    public class SlideshowControllerSelector : DefaultHttpControllerSelector
    {
        private HttpConfiguration _config;
        public SlideshowControllerSelector(HttpConfiguration config)
          : base(config)
        {
            _config = config;
        }
        ...
        private string GetVersionFromMediaType(HttpRequestMessage request)
        {
            ...
            var ex = new Regex(@"application\/vnd\.Slideshow\.([a-z]+)\.v([0-9]+)\+json", RegexOptions.IgnoreCase);
        ...
        {
            const string HEADER_NAME = "X-Slideshow-Version";
